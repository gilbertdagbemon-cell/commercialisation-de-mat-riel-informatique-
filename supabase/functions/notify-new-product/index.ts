...
Deno.serve(async req => {
  ...
  if (!supabaseUrl || !serviceKey) return json(req, { error: 'Server configuration error' }, 500);
  ...
  if (!resendKey) return json(req, { error: 'RESEND_API_KEY is not configured' }, 500);
  ...
  if (!body.product_id) return json(req, { error: 'product_id is required' }, 400);
  ...
  if (existing) return json(req, { ok: true, already_sent: true, sent_to_count: existing.sent_to_count || 0 });
  ...
  if (productError || !product) return json(req, { error: 'Product not found' }, 404);
  if (!product.is_published) return json(req, { ok: true, skipped: true, reason: 'Product is not published' });
  ...
  if (subscriberError) return json(req, { error: subscriberError.message }, 500);

  if (!subscribers?.length) {
    await adminClient.from('notification_logs').insert({ product_id: product.id, sent_to_count: 0, status: 'sent' });
    return json(req, { ok: true, sent_to_count: 0 });
  }
  ...
  return json(req, { ok: status !== 'failed', status, sent_to_count: sent, total: subscribers.length });
});
