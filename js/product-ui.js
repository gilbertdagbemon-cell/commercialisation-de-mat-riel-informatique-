...
export function renderProductsGrid(products, containerOrSelector = '#products-grid', options = {}) {
      ...
      <article class="card" data-product-id="${escapeHtml(product.id)}">
        ...
        <div class="card-body">
          ...
          <div class="card-foot">
            ...
            <button type="button" class="card-order" data-order-product-id="${escapeHtml(product.id)}" title="Commander sur WhatsApp" aria-label="Commander ${escapeHtml(title)}"><i class='bx bxl-whatsapp'></i></button>
          </div>
        </div>
      </article>`;
  }).join('');
  ...
  container.querySelectorAll('[data-order-product-id]').forEach(button => {
    button.addEventListener('click', () => {
      const product = window.__ordimarketProducts?.get(String(button.dataset.orderProductId));
      if (!product) return;
      openVendorPicker({
        channel: 'whatsapp',
        product: {
          title: String(product.title || 'Produit'),
          price: formatPriceFCFA(product.price),
        },
      });
    });
  });
